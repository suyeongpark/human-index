...
SQL_EXPERT_TREND = """
    ...
    JOIN expert_articles ea ON ea.id = eam.article_id
    JOIN expert_sources es ON es.id = ea.source_id
    WHERE eam.ticker_id = %s AND ea.published_date BETWEEN %s AND %s AND es.source_type = '{source_type}'
    GROUP BY {{date_grp}} ORDER BY 1
"""
...
def page_dashboard(start_date, end_date):
    """🏠 종합 대시보드"""
    page_header("🏠 종합 대시보드", "커뮤니티 · 전문가 · 뉴스 교차 분석")
    ...
    col_comm, col_rpt, col_news = st.columns(3)
...
def page_ticker_search(start_date, end_date):
    ...
    ticker_list = run_query(SQL_TICKER_LIST)
    ...
    st.markdown(
        '<hr style="border:none;height:1px;background:#e2e8f0;margin:0.5rem 0 1rem;">',
    ...
    stats = run_query(SQL_CHANNEL_STATS, (
    ...
    with col_comm:
        with st.container(border=True):
            st.markdown('<div class="channel-card-title">📢 커뮤니티</div>', unsafe_allow_html=True)
            query = SQL_COMM_TREND.replace("{{date_sel}}", date_sel_comm).replace("{{date_grp}}", date_grp_comm)
            comm_trend = run_query(query, (ticker_id, start_date, end_date))
    ...
    with col_rpt:
        with st.container(border=True):
            st.markdown('<div class="channel-card-title">🔬 리포트</div>', unsafe_allow_html=True)
            query = SQL_EXPERT_TREND.replace("{source_type}", "report").replace("{{date_sel}}", date_sel_expert).replace("{{date_grp}}", date_grp_expert)
            rpt_trend = run_query(query, (ticker_id, start_date, end_date))
    ...
    with col_news:
        with st.container(border=True):
            st.markdown('<div class="channel-card-title">📰 뉴스</div>', unsafe_allow_html=True)
            query = SQL_EXPERT_TREND.replace("{source_type}", "article").replace("{{date_sel}}", date_sel_expert).replace("{{date_grp}}", date_grp_expert)
            news_trend = run_query(query, (ticker_id, start_date, end_date))
