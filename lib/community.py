...
from lib.shared import (
    run_query, navigate_to, paginated_dataframe, period_tabs, show_period_range,
    render_top10_chart, render_surge_page, render_colored_line_chart,
    page_header, pagination_bar,
    PERIOD_OPTIONS, PERIOD_SQL, PERIOD_DAYS, PAGE_SIZE,
...
def page_overview(start_date, end_date):
    """📈 커뮤니티 개요"""
    page_header("📢 커뮤니티 대시보드", "커뮤니티 주식 언급량 분석 (대중 의견)")
    ...
    # KPI 카드 (선택된 기간 기준)
    kpi = run_query(SQL_KPI, (period_start, end_date, period_start, end_date, period_start, end_date, period_start, end_date))
    ...
    sentiment_trend = run_query(query, (start_date, end_date))
    ...
    top_tickers = run_query(SQL_TOP10, (top_start, top_end))
    render_top10_chart(top_tickers)


def page_mention_ranking(start_date, end_date):
    """📊 언급량 랭킹"""
    page_header("📊 종목 언급량 랭킹", "기간별 종목 언급 빈도 및 감성 점수 랭킹")

    period = period_tabs("comm_ranking_period", include_daily=True)
    days = PERIOD_DAYS[period]
    period_start = max(start_date, end_date - timedelta(days=days))
    show_period_range(period_start, end_date)
...
def page_surge_ranking(start_date, end_date):
    ...
    render_surge_page(
        ...
        period_key="surge_period",
        end_date=end_date,
        start_date=start_date,
        page_key="pg_surge_ranking",
    )


def page_author_ranking(start_date, end_date):
    """👤 작성자 랭킹"""
    page_header("👤 활발한 작성자 랭킹", "게시글 수 기준 상위 작성자 목록")

    authors = run_query(SQL_AUTHOR_RANKING, (start_date, end_date))
...
def page_post_list(start_date, end_date):
    ...
    with col2:
        comms = run_query(SQL_COMMUNITY_LIST, (start_date, end_date))
    ...
    # 총 건수
    if filter_symbol:
        count_df = run_query(SQL_POST_COUNT_FILTERED.format(where=where), params)
    else:
        count_df = run_query(SQL_POST_COUNT.format(where=where), params)
    total = int(count_df["cnt"].iloc[0]) if len(count_df) > 0 else 0

    # 데이터 조회
    query_params = params + [PAGE_SIZE, offset]
    if filter_symbol:
        posts_df = run_query(SQL_POSTS_FILTERED.format(where=where), query_params)
    else:
        posts_df = run_query(SQL_POSTS.format(where=where), query_params)
    ...
    pagination_bar(page, total_pages, "posts_page", "posts", show_refresh=True)
