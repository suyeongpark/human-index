...
def pagination_bar(page, total_pages, session_key, prefix, show_refresh=False):
    """통일된 페이지네이션 바 (⏮ ◀ 1/N ▶ ⏭ 🔄)"""
    cols = st.columns([6, 1, 1, 3, 1, 1, 6, 1] if show_refresh else [8, 1, 1, 3, 1, 1, 8])
...
# ─── 기간별 집계 헬퍼 ──────────────────────────────────
PERIOD_OPTIONS = ["주간", "월간", "분기", "반기", "연간"]            # 급상승 (비교 분석)
PERIOD_OPTIONS_WITH_DAILY = ["일간", "주간", "월간", "분기", "반기", "연간"]  # 랭킹/언급량
...
PERIOD_SQL = {
    ...
    "분기": {"select": "DATE_TRUNC('quarter', {date_col})::date", "group": "DATE_TRUNC('quarter', {date_col})"},
    "반기": {"select": "DATE_TRUNC('quarter', {date_col})::date", "group": "DATE_TRUNC('quarter', {date_col})"},
    "연간": {"select": "DATE_TRUNC('year', {date_col})::date", "group": "DATE_TRUNC('year', {date_col})"},
}

# TOP 10 등 집계에 사용할 최근 기간 (일 수)
PERIOD_DAYS = {"일간": 1, "주간": 7, "월간": 30, "분기": 90, "반기": 180, "연간": 365}
