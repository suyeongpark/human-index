...
크롤러는 중복 판정을 위해 최근 URL을 미리 로드하지만, `max_pages`와 `posts_per_page`에 맞춰 필요한 만큼만 가져옵니다. 얕은 수집(`max_pages=1`)에서는 기본 100개만 조회하여 DB egress를 줄입니다.

Analyzer는 미분석 게시글을 한 번에 모두 내려받지 않고 `ANALYZE_BATCH_SIZE` 단위로 처리합니다. 기본값은 1000이며, 환경변수로 조정할 수 있습니다.
...
- `check_fmkorea_health.py`는 연속 `status=430`을 `crawler_health_alerts`에 기록하고, 최신 응답이 `200`이면 열린 알림을 자동으로 처리 완료합니다.
